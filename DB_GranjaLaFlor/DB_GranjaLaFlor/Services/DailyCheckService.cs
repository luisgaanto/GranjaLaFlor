 ...
 * All calculated values are generated by RecalculateDailyChecksAsync.
 */
        public async Task UpdateAsync(DailyCheckFormViewModel model)
        {
            /*
             * Business Validation | Daily Check Week
             * Confirms that the submitted week belongs to the values
            ...
            /*
             * Stores the previous Brood identifier because the Daily Check
             * may be moved to a different Brood during the update.
             */
            var previousBroodId =
            ...
            /*
             * Business Validation | Duplicate Daily Check
             * Prevents another active Daily Check from using the same
             * Brood, week and day combination.
             *
             * The current Daily Check is excluded from this validation.
             */
            var duplicateExists =
                await _context.DailyChecks
                    .AsNoTracking()
            ...
            /*
             * Entity Mapping | Daily Check
             * Updates only the user-entered values and relationships.
             *
             * Calculated properties are updated later by
             * RecalculateDailyChecksAsync.
             */
            existingDailyCheck.DailyCheckDate = model.DailyCheckDate.Date;

            existingDailyCheck.DailyCheckWeek = model.DailyCheckWeek;

            existingDailyCheck.DailyCheckDay = model.DailyCheckDay;

            existingDailyCheck.NaturalMortality = model.NaturalMortality;

            existingDailyCheck.SelectQuantity = model.SelectQuantity;

            existingDailyCheck.ConsumptionQuintals = model.ConsumptionQuintals;
            ...
            existingDailyCheck.BroodId = model.BroodId;

            existingDailyCheck.IncomeConcentrateId = incomeConcentrate.IncomeConcentrateId;

            /*
             * Database Transaction | Update and Recalculate
             * Ensures that the Daily Check update and recalculations are
             * completed as a single operation. If one fails, no changes are saved.
             *
             * Source:
            ...
            {
                /*
                 * Saves the user-entered values and relationships before
                 * recalculating the affected Brood records.
                 */
                await _context.SaveChangesAsync();

                /*
                 * Recalculates the previous Brood because the Daily Check
                 * may have been moved to a different Brood.
                 */
                await RecalculateDailyChecksAsync(
                    previousBroodId);

                /*
                 * Recalculates the new Brood only when the Daily Check
                 * was moved from its previous Brood.
                 */
                ...
                /*
                 * Database Transaction | Commit
                 * Confirms the Daily Check update and all related recalculations.
                 */
                await transaction.CommitAsync();
            }
            catch
            {
                /*
                 * Database Transaction | Rollback: Reverts every change performed inside the transaction when any update or recalculation fails.
                 */
                await transaction.RollbackAsync();

                /*
                 * Error Propagation | Original Exception: Sends the same exception to the Controller so it can be logged and handled without losing the original error details.
                 */
                throw;
            }
        }
